...
class AnalyzeBody(BaseModel):
    ...
    upload_id: Optional[str] = None  # for Test Mode file uploads
    title: Optional[str] = None
    description: Optional[str] = None
    platform: Optional[str] = None  # youtube | tiktok | reels | upload
...
class AnalysisDoc(BaseModel):
    ...
    description: Optional[str] = None
    duration_sec: Optional[int] = None
    outlier_multiplier: float = 1.0  # placeholder, since no real metrics
    niche: Optional[str] = None

    # Pipeline outputs
    perception: Optional[Dict[str, Any]] = None  # transcript, shots, audio, summary
    scorecard: Optional[Dict[str, Any]] = None  # components[], composite
    scripts: List[Dict[str, Any]] = Field(default_factory=list)  # 10 scripts (study)
    alignment: Optional[Dict[str, Any]] = None  # good/bad/ugly + composite (test)
    detected_angle: Optional[str] = None
    trend_comparison: Optional[Dict[str, Any]] = None
    overall_score: Optional[float] = None
    band: Optional[str] = None  # loser|baseline|momentum|winner
    post_id: Optional[str] = None
    as_of: Optional[str] = None
    sent_to_swarm_command: bool = False

    # Sharing
    shared_slug: Optional[str] = None  # if set, the analysis is publicly viewable at /r/{slug}
...
class RateScriptBody(BaseModel):
    scores: Dict[str, int] = Field(default_factory=dict)  # per criterion 1..5
    chosen: bool = False
